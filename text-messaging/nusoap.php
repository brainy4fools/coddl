...
class nusoap_base {
    ...
    */
    ...
    // TODO: add an option to automatitioy run utf8_encode on $body and $headers
	...
	if($style == 'rpc') {
		$ns_string = ' SOAP:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"' . $ns_string;
	}
...
class nusoapclient extends nusoap_base  {
	...
	* @access   public
	*/
	function nusoapclient($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false){
	...
	*/
	function call($operation,$params=array(),$namespace='',$soapAction='',$headers=false,$rpcParams=null){
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			if($opData['input']['use'] == 'literal') {
				...
				if(isset($opData['output']['encodingStyle']) && $encodingStyle != $opData['output']['encodingStyle']) {
					$payload .= (' SOAP:encodingStyle="' . $opData['output']['encodingStyle'] . '"');
				}										
				$payload .= ('>' . $this->wsdl->serializeRPCParameters($operation,'input',$params).
		...
		//die("vinay".$this->getHTTPBody($soapmsg));
